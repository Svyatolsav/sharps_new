...
namespace Task8
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            InitializeComponent();
            AddActor(new InterfaceContainer(new Pass1(), $"Пассажир#{lCounter++}"));
            AddActor(new InterfaceContainer(new Pass2(), $"Пассажир#{lCounter++}"));
            for (int i = 0; i < 3; i++)
            {
                AddModel();
            }
        ...
        {
            AddActor(new Bus($"Автобус#{bCounter++}"));
        }
